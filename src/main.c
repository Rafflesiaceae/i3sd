...
struct dbus_transport {
    /* One native connection is shared by every live block in this scope. */
    struct app *app;
    sd_bus *bus;
    uint64_t cookie;
    uint64_t epoch;
...
struct app {
    ...
    struct dbus_transport dbus_transports[2];
...
static int lua_dbus_handle_cancel(lua_State *lua) {
    ...
    if (handle->kind == LUA_DBUS_MATCH && handle->value.match != NULL) {
        ...
        struct generation *generation = handle->value.call.generation;
...
static int lua_context_dbus(lua_State *lua) {
    struct lua_context *context = check_context(lua, 1);
    const char *scope = luaL_checkstring(lua, 2);
...
static int lua_dbus_match(lua_State *lua) {
    ...
    struct generation *generation = bus->block->generation;
    luaL_checktype(lua, 2, LUA_TTABLE);
    luaL_checktype(lua, 3, LUA_TFUNCTION);
...
static bool decode_dbus_value(sd_bus_message *message, lua_State *lua,
    ...
    if (type == 'y') {
        ...
        int64_t value = 0;
        result = sd_bus_message_read_basic(message, type, &value);
        push_int64(lua, value);
    } else if (type == 't') {
        uint64_t value = 0;
        result = sd_bus_message_read_basic(message, type, &value);
        push_uint64(lua, value);
    } else if (type == 'd') {
        double value = 0;
        result = sd_bus_message_read_basic(message, type, &value);
        lua_pushnumber(lua, value);
...
static void register_lua_api(struct generation *generation) {
    ...
    lua_newtable(lua);
    static const luaL_Reg dbus_bus_methods[] = {
        {"match", lua_dbus_match},
        {"call", lua_dbus_call},
        {"on_connect", lua_dbus_on_connect},
        {NULL, NULL},
    };
    luaL_register(lua, NULL, dbus_bus_methods);
    lua_setfield(lua, -2, "__index");
    lua_pop(lua, 1);
    ...
    lua_newtable(lua);
    static const luaL_Reg dbus_message_methods[] = {
    ...
    luaL_register(lua, NULL, dbus_message_methods);
    lua_setfield(lua, -2, "__index");
    lua_pop(lua, 1);
...
static bool dbus_callback_allowed(const struct block *block) {
    return !block->faulted &&
           block->generation->app->current == block->generation;
}

static bool finish_dbus_callback(struct block *block, int arguments,
                                 const char *phase) {
    lua_State *lua = block->generation->lua;
...
static bool dbus_transport_needed(const struct app *app,
                                  enum dbus_scope scope) {
    if (app->current == NULL) {
        return false;
    }
...
static void reap_dbus_calls(struct generation *generation) {
    if (generation == NULL) {
        return;
    }
...
static void reset_dbus_slots(struct generation *generation,
...
static void activate_dbus_operations(struct dbus_transport *transport) {
    struct generation *generation = transport->app->current;
...
static int epoll_timeout_ms(struct app *app, uint64_t now_ns) {
    ...
    for (size_t index = 0; index < 2; index++) {
        struct dbus_transport *transport = &app->dbus_transports[index];
        if (!dbus_transport_needed(app, transport->scope)) {
            continue;
        }
        if (transport->bus == NULL) {
...
static bool initialize_runtime(struct app *app, const sigset_t *signal_mask) {
    ...
    app->dbus_transports[0] = (struct dbus_transport){
        .app = app,
        .registered_fd = -1,
        .scope = DBUS_SCOPE_SYSTEM,
    };
    app->dbus_transports[1] = (struct dbus_transport){
        .app = app,
        .registered_fd = -1,
...
static void run_event_loop(struct app *app) {
    ...
    while (app->running) {
        ...
        reconcile_dbus_transport(&app->dbus_transports[0], now_ns);
        reconcile_dbus_transport(&app->dbus_transports[1], now_ns);
        int event_count;
        do {
        ...
        for (int index = 0; index < event_count; index++) {
            ...
            case SOURCE_SPAWN:
                ...
                break;
            default:
                if (events[index].data.u64 == app->dbus_transports[0].cookie &&
                    app->dbus_transports[0].registered_fd >= 0) {
                    process_dbus_transport(&app->dbus_transports[0],
                                           monotonic_now_ns());
                } else if (events[index].data.u64 ==
                               app->dbus_transports[1].cookie &&
                           app->dbus_transports[1].registered_fd >= 0) {
                    process_dbus_transport(&app->dbus_transports[1],
                                           monotonic_now_ns());
                }
                break;
            }
        ...
        process_dbus_transport(&app->dbus_transports[0], now_ns);
        process_dbus_transport(&app->dbus_transports[1], now_ns);
...
static void app_destroy(struct app *app) {
    ...
    if (app->dbus_transports[0].bus != NULL) {
        close_dbus_transport(&app->dbus_transports[0], false);
    }
    if (app->dbus_transports[1].bus != NULL) {
        close_dbus_transport(&app->dbus_transports[1], false);
    }
    if (app->spawn.active) {
